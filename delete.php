<?php
session_start();

require_once 'vendor/autoload.php'; //including the composer's autoloader file in the script

if (!isset($_SESSION["id"])) {
    header("Location: login.php"); //redirecting the user to the login page if not logged in (session id variable not set)
    exit;
} else {
    $isConfirmed = isset($_POST["confirm"]) && $_POST["confirm"] === "yes";

    if ($_SERVER["REQUEST_METHOD"] === "POST") { //if a post request is sent to the server

        include 'db_connection.php'; //opening db connection

        // Checking if the user confirmed the account deletion
        if (isset($_POST["confirm"]) && $_POST["confirm"] === "yes") {
            // Get the user's ID
            $userId = $_SESSION["id"];

            // Deleting the user's account, first from the user preferences table due to the tables being linked.
            $sqlDeletePreferences = "DELETE FROM user_preferences WHERE user_id = ?";
            $stmtDeletePreferences = $mysqli->prepare($sqlDeletePreferences);
            $stmtDeletePreferences->bind_param("i", $userId);
            $stmtDeletePreferences->execute();

            // Deleting the user's record from the users table
            $sqlDeleteUser = "DELETE FROM users WHERE id = ?";
            $stmtDeleteUser = $mysqli->prepare($sqlDeleteUser);
            $stmtDeleteUser->bind_param("i", $userId);
            $stmtDeleteUser->execute();

            // Closing the database connection
            $mysqli->close();

            // Logout the user and destroy the session
            session_destroy();

            // Redirecting to the confirmation page
            header("Location: deleted_confirmation.html");
            exit;
        }
    }

    // Rendering the delete.twig template
    $loader = new \Twig\Loader\FilesystemLoader(__DIR__);
    $twig = new \Twig\Environment($loader);

    echo $twig->render('delete.twig', [
        'confirmForm' => !$isConfirmed,
    ]);
}
?>